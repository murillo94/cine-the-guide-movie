...
export default class ListItemTeam extends React.PureComponent {
  render() {
    ...
    if(type === 'character' || type === 'job') {
      return (
        <TouchableOpacity activeOpacity={0.5} style={styles.containerCast} onPress={() => actionTeamDetail(item.id)}>
          {type === 'character' &&
          ...
          <Image source={getImageApi(item.profile_path)} 
    ...
    return (
      <View style={styles.containerCast}>
        <Image source={getImageApi(item.logo_path)}
          style={styles.productionCompaniesPhoto} resizeMode="contain" />
